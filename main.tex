...
\begin{document}
	...
	\subsubsection[A integral representation not involving $\text{exp}\left(x\right)$ in the integrand]{$E_1\left(x\right)=e^{-x}\int_{0}^{1}\frac{1}{x-\ln{t}}dt$, source is eq. (4) in sec. 3.3 of \cite{geller1969table}}
	...
	\subsubsection[A integral of $E_1\left(x\right)$, involving the exponential function]{$\int_{0}^{d}e^{ax}E_1\left[b\left(x+c\right)\right]dx=\frac{e^{-bc}}{a}\left(e^{\left(a-b\right)d}\left\{D\left[b\left(c+d\right)\right]-D\left[\left(-a+b\right)\left(c+d\right)\right]\right\}-D\left[bc\right]+D\left[\left(-a+b\right)c\right]\right)$}
	...
	\subsubsection[A integral of $E_1\left(x\right)$, involving the exponential function]{$\int_{0}^{\infty}e^{ax}E_1\left[b\left(x+c\right)\right]dx=\frac{1}{a}e^{-bc}\left\{-D\left[bc\right]+D\left[c\left(-a+b\right)\right]\right\}$}
